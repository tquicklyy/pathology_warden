...
public class SceneStartAnalysisWithAccController {
    ...
    private void startAnalysis(MouseEvent event) throws IOException {
        if(isBeforeAnalysisStartAnalysisButtonRed) {
            ...
            currentStage.setScene(newScene);
            createAreas(sceneEndAnalysisWithAccountController.getEndAnalysisGeneralImageView());
            for (int i = 0; i < numAreas; i++) {
                for (Circle point : pointsList.get(i)) {
                    int finalI = i;
                    point.setOnMouseDragged(ev -> handlePointDrag(ev, sceneEndAnalysisWithAccountController.getEndAnalysisGeneralImageView(),finalI));
                }
            }
            pointsList.forEach(el->sceneEndAnalysisWithAccountController.getEndAnalysisGeneralAnchorPane().getChildren().addAll(el));
            linesList.forEach(el->sceneEndAnalysisWithAccountController.getEndAnalysisGeneralAnchorPane().getChildren().addAll(el));
        }
    }
